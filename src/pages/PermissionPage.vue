        ...
        <q-card-section>
          <q-form @submit="savePermission" class="q-gutter-md">
            ...
            <q-select
              ...
              :options="groupSearch.filteredOptions.value"
              ...
              use-input
              input-debounce="300"
              @filter="(val, update) => groupSearch.filterOptions(val, update)"
              @popup-show="groupSearch.resetFilter"
              :rules="[(val) => !!val || $t('pages.permissions.required')]"
            />
...
// Searchable selects
const groupSearch = useSearchableSelect(groupOptions)
...
const fetchGroups = async () => {
  ...
  try {
    ...
    groupSearch.initializeOptions(groupOptions.value)
  } catch {
    $q.notify({
      type: 'negative',
